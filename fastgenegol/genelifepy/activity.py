...
# gene1 gene1_cnt gene2 gene2_cnt ...
# for all genes in the population that generation.
...
#        trdat = [(colors[xx],activity[xx]) for xx in activity if activity[xx] is not None]   # assemble data for trace
        trdat = [(colors[xx],activity[xx]) for xx in activity if xx in actinc]   # assemble data for trace don't show if dead this generation
        if len(trdat) > maxact:
