  ...
  def ProcessEvent(self, event):
    ...
    if event.type == pygame.QUIT:
      ...
      if (event.key == K_DOWN or event.key == K_UP or 
          event.key == K_LEFT or event.key == K_RIGHT): # Pan.
        self._world.ShiftView(event.key, max(self._width, self._height) // 20)
      elif event.key == K_MINUS or event.key == K_KP_MINUS: # Slow down.
        if (self._generations_per_update > 1):
          self._generations_per_update >>= 1
        else:
          self._ticks_per_update <<= 1
      elif event.key == K_EQUALS or event.key == K_KP_PLUS: # Speed up.
        if self._ticks_per_update > 1:
          self._ticks_per_update >>= 1
        else:
          self._generations_per_update <<= 1
      elif event.key == K_SPACE: # Pause.
        self._paused = not self._paused
      elif event.key == K_PAGEDOWN: # Zoom in.
        self._world.ZoomIn()
      elif event.key == K_PAGEUP: # Zoom out.
        self._world.ZoomOut()
      elif event.key == K_q and (pygame.key.get_mods() & KMOD_CTRL): # Quit.
        sys.exit()
